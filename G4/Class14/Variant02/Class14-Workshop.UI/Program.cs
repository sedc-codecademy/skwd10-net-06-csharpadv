...
namespace Class14_Workshop.UI
{
    ...
    /// <summary>
    /// Variant02 - preserve "rich" domain models encapsulation
    ///
    /// This obviously is the "proper" way to do domain modelling - give the minimum
    /// possible access to domain models while still having full control of them for
    /// the purposes of implementing the business scenarios. This complicates matters
    /// on persistence side, especially coming from an implementation that was initially
    /// designed with an in-memory storage in mind. We need to do some specific Newtonsoft.Json
    /// configuration of the entities so serialization and deserialization can happen without
    /// issues.
    /// 
    /// We also need to hack our way to making Ids be generated from the database/repository,
    /// otherwise we would be breaking the single responsibility of the other layers. Check out
    /// the CustomIdFactory entity static properties as well as the implementation of
    /// <see cref="FileSystemGenericRepository{T}"/> for more details.
    /// </summary>
    class Program
    {
